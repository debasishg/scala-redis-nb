...
private [redis] class RedisConnection(remote: InetSocketAddress, settings: RedisClientSettings) 
  ...
  def unconnected: Receive = {
    ...
    case CommandFailed(c: Connect) =>
      if (reconnectionSchedule.maxAttempts > 0 && reconnectionSchedule.attempts < reconnectionSchedule.maxAttempts ) {
        val delayMs = reconnectionSchedule.nextDelayMs
        log.error("Connect failed for {} with {}. Reconnecting in {} ms with attempt {}  ... ", c.remoteAddress, c.failureMessage, delayMs,reconnectionSchedule.attempts)
