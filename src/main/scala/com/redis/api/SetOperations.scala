...
trait SetOperations { this: RedisOps =>
  ...
  def sadd[A](key: String, value: A, values: A*)(implicit timeout: Timeout, write: Write[A]) =
    clientRef.ask(SOp[A](add, key, value, values:_*)).mapTo[SOp[A]#Ret]

  // SREM (VARIADIC: >= 2.4)
  // Remove the specified members from the set value stored at key.
  def srem[A](key: String, value: A, values: A*)(implicit timeout: Timeout, write: Write[A]) =
    clientRef.ask(SOp[A](rem, key, value, values:_*)).mapTo[SOp[A]#Ret]

  // SPOP
  // Remove and return (pop) a random element from the Set value at key.
  def spop[A](key: String)(implicit timeout: Timeout, read: Read[A]) =
    clientRef.ask(SPop[A](key)).mapTo[SPop[A]#Ret]

  // SMOVE
  // Move the specified member from one Set to another atomically.
  def smove[A](sourceKey: String, destKey: String, value: A)(implicit timeout: Timeout, write: Write[A]) =
  ...
  def sismember[A](key: String, value: A)(implicit timeout: Timeout, write: Write[A]) =
    clientRef.ask(∈[A](key, value)).mapTo[(∈[A])#Ret]

  // SINTER
  // Return the intersection between the Sets stored at key1, key2, ..., keyN.
  def sinter[A](key: String, keys: String*)(implicit timeout: Timeout, read: Read[A]) =
  ...
  def sunion[A](key: String, keys: String*)(implicit timeout: Timeout, read: Read[A]) =
  ...
  def sdiff[A](key: String, keys: String*)(implicit timeout: Timeout, read: Read[A]) =
  ...
  def smembers[A](key: String)(implicit timeout: Timeout, read: Read[A]) =
    clientRef.ask(SMembers(key)).mapTo[SMembers[A]#Ret]

  // SRANDMEMBER
  // Return a random element from a Set
  def srandmember[A](key: String)(implicit timeout: Timeout, read: Read[A]) =
    clientRef.ask(SRandMember(key)).mapTo[SRandMember[A]#Ret]

  // SRANDMEMBER
  // Return multiple random elements from a Set (since 2.6)
  def srandmember[A](key: String, count: Int)(implicit timeout: Timeout, read: Read[A]) =
